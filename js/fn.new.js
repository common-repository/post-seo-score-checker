...
{
	$("#pssc-meta-box").removeClass("closed");
...
function checkStatus()
{
	...
	var nonce_code = $("#psscNonceSecurity").html();
	$.ajax({
		url : adminURL + "post.php",
		type: "post",
		data: {"pssc_check_status": 1, 'nonce_security' : nonce_code},
...
function checkSEO_step2()
{
	...
	if(previewHTML < 1)
	{
		...
		alert('SEO Score checker, can only be checked when you will publish your post or save it as draft');
	...
	$.ajax({
		...
		success: function(data)
		{
			...
			if(previewHTML.length > 100)
			{
				...
				alert("Please Save Post as draf first, before checking SEO Score for this post");
			}
		},
...
{
	$("#pssc-meta-box").removeClass("closed");
	var elemOff = $("#pssc-meta-box").offset().top;
	elemOff = elemOff-100;
	activeActions = 1;
...
function checkPostUrl()
{
	...
	var postSlug = $("#editable-post-name-full").html();
	var percent = 0;
...
function checkDensity(text)
{
	...
	var result = true;
	activeActions = 1;
	$("#densityStatus").html("<center>Loading: Checking Keywords Density.....</center>");
	var nonce_code = $("#psscNonceSecurity").html();
	$.ajax({
		url : adminURL + "post.php",
		type: "post",
		data: {data:text, pssc_check_density : 1, 'nonce_security' : nonce_code},
		dataType:"json",
		async: true, 
		success: function(resp)
		{
			...
			if(resp.error == "1")
			{
				var htmlIn = '<span class="notice none"><p>'
				   + '<b class="labelN">Keywords Density : </b><br>'
				   +  'Keywords Density Not Found..' 
				   +  '</p></span>';
	...
	{		
		...
		function sendLinksReq()
		{
				if(start < linksArray.length)
				{
					...
					$.ajax({
						url : adminURL + "post.php",
						data: {url:linksArray[start], "pssc_check_broken" : 1, 'nonce_security': nonce_code},
						type: "post",
		...
		var infoMsg = '<span class="alert alert_info">No Link found in your post content.</span>';
		
		$("#linksStatus").html(infoMsg);
...
function ppsSaveMeta(data)
	{
		...
		$.ajax({
			type: 'POST',
			url : adminURL + "post.php",
			data : {"data" : data, "id": postId, "pps_post_meta" : 1},
			async:true
		});
	}
...
function sendRequests(innerText, accKey)
{
	...
	plagBtn.addClass("disable");
		...
		var parts = innerText.replace(/([.?!])\s*(?=[a-z]|[A-Z])/g, "$1|").split("|");
		...
		$.each(parts, function(index, value) {
				...
				if(tmpVar.length < 55 && value.length < 55)
				{
					tmpVar += value + " ";
		...
		*/
		...
		$.each(values, function(key, sentence){
				if(skiplimit < 10)
				{
					if(skiplimit < values.length)
					{
						tmpsndArr += sentence + '[--ppsbreak--]';
					} else {
						tmpsndArr += sentence;
					}
		...
		function doRequest(index) {
			...
			$.ajax({
				type: 'POST',
				url : adminURL + "post.php",
				data : {"query-list" : sendArr[index], "key" : accKey, "pps_check_plag" : 1},
				async:true,
				...
				success: function(response){
					...
					if(resp.data.unique == "true")
					{
						...
						+ '<span class="txt">' + values[index] + ' <b> - Unique</b></span>'
						+ '</span>';
						...
						+ '<span class="txt">' + values[index] + ' <b>- plagiarized</b></span>'
						+ '<textarea style="display:none;" id="ppscomData-'+index+'">'+response+'</textarea>'
						+ '<span class="check"><span class="button button-primary ppsCompare" id="ppscomBtn-'+index+'">Compare</span></span>';
					}
					
					$(".resultsBars").append(alertHtml);
					
					totalChecked = totalChecked+uparts;
					var roundTotal = totalChecked.toFixed(0);
					$("#totalCount").html(roundTotal);
					
					roundUnique = totalU.toFixed(0);
					$("#uniqueCount").html(roundUnique);
					
					var roundPlag = parseInt(roundTotal) - parseInt(roundUnique);
					
					//$("#uniqueBar").animate({"width" : roundUnique+"%"}, 500);
					
					$("#plagCount").html(roundPlag);
					$("#totalBar").animate({"width" : roundTotal+"%"}, 500);
					*/
					...
					$.each(resp.data, function( key, singleVal ) 
						{		
							var s_name_singlevalue = '';
							if(typeof singleVal.s_name != 'undefined')
							{
								s_name_singlevalue = singleVal.s_name;
							}
							
							var queryVal = singleVal.query;
							if(singleVal.query.length > 77)
							{
								queryVal = singleVal.query.substr(0,70) + '....';
							}
											
							if(singleVal.unique == "true")
							{
								
								alertHtml = '<span class="statBox uniqueSta">'
								+ '<span class="txt">' + queryVal + ' <b> - Unique</b></span>'
								+ '</span>';
								totalU = totalU+uparts;
							} else {
								isPlagOnce = 1;
								
								var alertHtml = '<span class="statBox plagSta">'
									+ '<span class="txt">'+queryVal+'  <b> - Plagiarized</b></span>'
									+ '<a class="btn-gray"  href = "'+remoteUrl+'compare/'+s_name_singlevalue+'" target="_blank">Compare</a></span>';		
							}
							...
							var roundPlag = parseInt(roundTotal) - parseInt(roundUnique);
							$("#plagCount").html(roundPlag);
							$("#totalBar").stop().animate({"width" : roundTotal+"%"}, 500);
