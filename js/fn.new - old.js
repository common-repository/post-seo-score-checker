...
function checkSEO_Grammar()
{
	...
	checkGrammar(postContent, accKey);
}
...
function windowScrolling()
{
	...
	var elemOff = $("#pps-meta-box").offset().top;
	elemOff = elemOff-100;
	activeActions = 1;
...
function checkGrammar(content, accKey)
{
	...
	$.ajax({
		url : adminURL + "post.php",
		type: "post",
		data: {data:content, pps_check_grammar : 1, key:accKey},
		...
		success: function(resp)
		{
			...
			var totalSpell = 0;
			var totalGrammar = 0;
			grammarHtml = '<span class="sec_heading">Grammar Checker</span>';
			
			if(typeof resp.error === 'undefined') 
			{
				grammarHtml += '<center><strong class="green">Great!</strong> No Grammar or Spelling Error Found in your Post</center>';
			} else {
				for(i =0; i < resp.error.length; i++)
				{
					if(resp.error[i].type == 'spelling')
					{
						totalSpell++;
					} else {
						totalGrammar++;
				...
				grammarHtml += '<table class="densityTabel" width="100%">'
						+ '<tr class="tr_head1"><td width="80%">Mistake/Error Type</td><td width="20%">Status</td></tr>'
						+ '<tr><td>Spelling Errors</td>'
						+'<td><strong class="red">'+totalSpell+' Error(s) Found<strong></td></tr>'
						+ '<tr><td>Grammar / Styling Errors</td>'
						+'<td><strong class="red">'+totalGrammar+' Error(s) Found<strong></td></tr>'
						+ '<tr><td colspan="2" align="center">'
						
						+ '<span class="button button-primary button-large" id="viewGrammarDetails">View Details</span>'
						+ '</td></tr>'
						+ '</table>';
			}
			
			$("#grammarStatus").html(grammarHtml);
			$("#gDForm").append('<textarea name="data" style="display:none;">'+content+'</textarea>');
			$("#gDForm").append('<textarea name="resp" style="display:none;">'+mainResp+'</textarea>');
